  ...
  <div class="panel panel-default panel-primary"  style="max-width: 500px;margin: 0 auto;">
    ...
    <div class="panel-body">
      ...
      <div class="alert alert-danger fade in">
        ...
        <strong>{$Captions->GetMessageString('ErrorsDuringUpdateProcess')}</strong><br/>
        {$Grid->GetErrorMessage()}
      </div>
      {/if}
      <p class="help-block">{$Captions->GetMessageString('RequiredField')} <font color="#FF0000">*</font></p>
      <form name="editform" id="editform" enctype="multipart/form-data" method="POST" action="{$Grid->GetEditPageAction()}">
    ...
    <div class="panel-footer">
      <input class="btn btn-primary" type="button" value="{$Captions->GetMessageString('Save')}" name="submit1" onclick="if (ValidateControls()) document.editform.submit();"/>
...
<style type="text/css">
  ...
  #editform .row .caption-text {ldelim}
    height: 34px;
    line-height: 34px;
